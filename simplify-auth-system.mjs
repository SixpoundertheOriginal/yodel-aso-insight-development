...
console.log('\n=== Simplifying Auth System ===\n');

// Step 1: Create MEMBER role permissions
console.log('1️⃣  Setting up MEMBER role permissions...');
...
const MEMBER_FEATURES = [
  ...
  'profile_management',
  'preferences',
  'app_core_access' // Base app access
];

console.log('   MEMBER role will have access to:', MEMBER_FEATURES);

// Delete existing MEMBER role permissions
...
if (deleteError && deleteError.code !== 'PGRST116') { // PGRST116 = no rows found (ok)
  console.log('   ⚠️  Could not delete old MEMBER permissions:', deleteError.message);
}

// Insert MEMBER role permissions
for (const feature of MEMBER_FEATURES) {
...
// Step 3: Update users to MEMBER role (except Igor)
...
for (const userRole of yodelUsers || []) {
  ...
  if (userRole.user_id === igorUserId) {
    ...
    // Update everyone else to MEMBER
    const { error } = await supabase
      .from('user_roles')
      .update({ role: 'MEMBER' })
      .eq('user_id', userRole.user_id)
    ...
    if (error) {
      console.error(`   ❌ Failed to update ${email} to MEMBER:`, error.message);
    } else {
...
console.log('✅ All Yodel Mobile users (except Igor) have MEMBER role');
