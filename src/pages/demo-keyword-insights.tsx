...
type DemoResponse = {
  ...
  summary: { coverage_score: number; priority_terms: number };
  findings: { id: string; term: string; gap: string }[];
  recommendations: { id: string; steps: string[] }[];
};

export default function DemoKeywordInsightsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/demo-keyword-insights-summary`, {
  ...
  return (
    <MainLayout>
      <div className="p-6 space-y-6">
        <h1 className="text-2xl font-bold text-foreground">Keyword Insights (Demo)</h1>
          ...
          <>
            ...
            <Card className="bg-background/60 border-border">
              <CardContent className="p-4">
                ...
                <div className="text-foreground">Coverage score: {data.summary.coverage_score} · Priority terms: {data.summary.priority_terms}</div>
              </CardContent>
            </Card>
            <Card className="bg-background/60 border-border">
              <CardContent className="p-4">
                ...
                <ul className="list-disc pl-5 text-foreground text-sm">
                  {data.findings.map(f => (<li key={f.id}>{f.term} — {f.gap}</li>))}
                </ul>
              </CardContent>
            </Card>
            <Card className="bg-background/60 border-border">
