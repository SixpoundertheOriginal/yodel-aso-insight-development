...
import { Star, Download, Eye, Filter, SortAsc, Calendar as CalendarIcon, Smile, Meh, Frown, Brain, ChevronLeft, RefreshCw } from 'lucide-react';
...
const AppReviewDetailsPage: React.FC = () => {
  ...
  // Disable insights auto-fetching
  ...
  if (!appId) {
    return <Navigate to="/growth-accelerators/reviews" replace />;
  }

  // Load app metadata from URL params or localStorage
  useEffect(() => {
    const storedMetadata = localStorage.getItem(`app-metadata-${appId}`);
    if (storedMetadata) {
      try {
        setAppMetadata(JSON.parse(storedMetadata));
      } catch (e) {
        console.error('Failed to parse stored app metadata', e);
      }
    }
  }, [appId]);
  ...
  const handleLoadMore = () => {
    if (appId && hasMoreReviews && !reviewsLoading) {
      fetchReviews(appId, currentPage + 1, true);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    try {
      const filename = `reviews-${appMetadata?.name || appId}-${selectedCountry}-${Date.now()}`;
  ...
  const reviewIntelligence = useMemo(() => {
    if (!enhancedReviews || enhancedReviews.length === 0) {
      return { themes: [], featureMentions: [], issuePatterns: [] };
    }
  ...
  const actionableInsights = useMemo(() => {
    if (!enhancedReviews || enhancedReviews.length === 0) {
      return { priorityIssues: [], improvements: [], alerts: [] };
    }
  ...
  const periodTotal = useMemo(() => {
    if (!fromDate && !toDate) return reviews.length;
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Link to="/growth-accelerators/reviews" className="hover:text-foreground flex items-center gap-1">
            <ChevronLeft className="w-4 h-4" />
            Back to Review Search
          </Link>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {appMetadata?.icon && (
              <img src={appMetadata.icon} alt={appMetadata.name} className="w-16 h-16 rounded-lg shadow" />
            )}
            <div>
              <h1 className="text-3xl font-bold">{appMetadata?.name || `App ${appId}`}</h1>
              <p className="text-muted-foreground">{appMetadata?.developer || 'Reviews & Analytics'}</p>
              {appMetadata?.rating && (
                <div className="flex items-center gap-2 mt-1">
                  <StarRating rating={Math.round(appMetadata.rating)} />
                  <span className="text-sm text-muted-foreground">
                    {appMetadata.rating.toFixed(2)} / 5 • {appMetadata.reviews?.toLocaleString() || 0} ratings
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Button variant="outline" size="sm" onClick={handleRefresh} disabled={reviewsLoading}>
              <RefreshCw className={`w-4 h-4 mr-2 ${reviewsLoading ? 'animate-spin' : ''}`} />
              Refresh
            </Button>
          </div>
        </div>
          ...
          <YodelCardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
              ...
              <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
                ...
                <div className="text-xl font-semibold">
                  {(appMetadata?.rating ?? 0).toFixed(2)}
                  <span className="text-xs text-muted-foreground"> / 5</span>
                </div>
                <div className="text-[10px] text-muted-foreground mt-1">{(appMetadata?.reviews ?? 0).toLocaleString()} ratings</div>
              </div>
              <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
                  ...
                  <BarChart data={ratingDistribution} margin={{ top: 8, right: 8, bottom: 0, left: 8 }}>
                    ...
                    <Bar dataKey="count">
                      {ratingDistribution.map((entry: any) => {
                        const label = entry.rating as string;
            ...
            {filteredReviews.length === 0 && !reviewsLoading && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>{reviews.length === 0 ? 'Loading reviews...' : 'No reviews match current filters'}</p>
              </div>
            )}
...
export default AppReviewDetailsPage;
