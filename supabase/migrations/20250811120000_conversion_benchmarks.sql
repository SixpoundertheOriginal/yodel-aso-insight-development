...
-- Policy allowing authenticated users to read benchmark data
CREATE POLICY "Allow authenticated users to read benchmarks"
ON public.conversion_benchmarks
FOR SELECT
TO authenticated
USING (true);

-- Policy allowing admins to manage benchmarks
CREATE POLICY "Allow admins to manage benchmarks"
ON public.conversion_benchmarks
FOR ALL
TO authenticated
USING (
...
-- Insert the complete benchmark dataset
INSERT INTO public.conversion_benchmarks (category, impressions_to_page_views, page_views_to_installs, impressions_to_installs) VALUES
